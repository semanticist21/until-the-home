...
class ListBuilder {
  ...
  Widget _buildListItem(
    ...
    if (counter != null && paragraphBuilder.searchController != null) {
      ...
      final query = paragraphBuilder.searchController!.query.toLowerCase();
      if (query.isNotEmpty) {
        final text = _extractTextFromInlines(item.children);
        if (text.toLowerCase().contains(query)) {
          debugPrint(
              'GENERATE DEBUG: Block $blockIndex (ListItem) contains "$query", matches: ${matches.length}, text: "${text.substring(0, text.length.clamp(0, 50))}..."');
        }
      }
  ...
  /// Helper to extract text from inlines for debug logging.
  String _extractTextFromInlines(List<DocxInline> inlines) {
    final buffer = StringBuffer();
