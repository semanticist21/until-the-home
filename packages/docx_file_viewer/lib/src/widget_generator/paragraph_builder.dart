...
class ParagraphBuilder {
  ...
  Widget build(DocxParagraph paragraph, {BlockIndexCounter? counter}) {
    ...
    if (counter != null && searchController != null) {
      ...
      final text = _extractText(paragraph);
      final query = searchController!.query.toLowerCase();
      if (query.isNotEmpty && text.toLowerCase().contains(query)) {
        debugPrint(
            'GENERATE DEBUG: Block $blockIndex (Paragraph) contains "$query", matches: ${matches.length}, text: "${text.substring(0, text.length.clamp(0, 50))}..."');
      }

      // Always register key for search navigation (not just when matches exist)
  ...
  /// Helper to extract text from paragraph for debug
  String _extractText(DocxParagraph paragraph) {
  ...
  Widget buildExcludingFloats(
    ...
    if (counter != null && searchController != null) {
      ...
      if (query.isNotEmpty && text.toLowerCase().contains(query)) {
        debugPrint(
            'GENERATE DEBUG: Block $blockIndex (FloatParagraph) contains "$query", matches: ${matches.length}, text: "${text.substring(0, text.length.clamp(0, 50))}..."');
      }
