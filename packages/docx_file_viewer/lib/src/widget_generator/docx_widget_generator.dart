...
class DocxWidgetGenerator {
  ...
  List<String> extractTextForSearch(DocxBuiltDocument doc) {
    ...
    debugPrint('EXTRACT DEBUG: Starting extraction...');
    ...
    // Body
    _extractFromBlocks(doc.elements, texts, counter, debug: true);
    ...
    debugPrint(
        'EXTRACT DEBUG: Extraction complete. Total blocks: ${texts.length}');
    return texts;
  }
  ...
  void _extractFromBlocks(
    ...
    for (final node in nodes) {
      if (node is DocxParagraph) {
        final text = _extractFromParagraph(node);
        final blockIndex = counter.value;
        texts.add(text);
        if (debug && text.toLowerCase().contains('inline')) {
          debugPrint(
              'EXTRACT DEBUG: Block $blockIndex (Paragraph) = "${text.substring(0, text.length.clamp(0, 50))}..."');
        }
        counter.increment();
      } else if (node is DocxTable) {
        for (final row in node.rows) {
          for (final cell in row.cells) {
            _extractFromBlocks(cell.children, texts, counter, debug: debug);
          }
        }
      } else if (node is DocxList) {
        for (final item in node.items) {
          // List item behaves like a paragraph
          final text = _extractFromInlines(item.children);
          final blockIndex = counter.value;
          texts.add(text);
          if (debug && text.toLowerCase().contains('inline')) {
            debugPrint(
                'EXTRACT DEBUG: Block $blockIndex (ListItem) = "${text.substring(0, text.length.clamp(0, 50))}..."');
          }
          counter.increment();
        }
      } else if (node is DocxSectionBreakBlock) {
