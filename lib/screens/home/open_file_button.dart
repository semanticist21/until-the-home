...
class _OpenFileButtonState extends State<OpenFileButton> {
  ...
  Future<void> _handleFilePick() async {
    if (_isPickingFile) {
      appLogger.d('[OPEN_FILE_BUTTON] Already picking file, ignoring tap');
      return;
    }
    ...
    try {
      ...
      appLogger.d(
        '[OPEN_FILE_BUTTON] Selected path: $path, size: $fileSize bytes',
      );
      ...
      if (gotenbergFormats.contains(fileType) && fileSize > maxSize) {
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text('변환 가능한 파일 크기는 25MB까지입니다.\n선택한 파일: $sizeMB MB'),
            actions: [
              TextButton(
      ...
      if (e.code == 'multiple_request') {
        appLogger.w('[OPEN_FILE_BUTTON] Cancelled by multiple request');
      } else {
        appLogger.e('[OPEN_FILE_BUTTON] Platform exception', error: e);
      }
    } catch (e) {
      appLogger.e('[OPEN_FILE_BUTTON] Error picking file', error: e);
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: _isPickingFile ? null : _handleFilePick,
          borderRadius: BorderRadius.circular(16),
          child: Padding(
